# Contributing to VA VistA Browser Automation
...
- **Broken selectors**: VA VistA updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real VA VistA instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.VA_VISTA_URL}/path/to/section`);
...
When you find working selectors for VA VistA, document them:

```javascript
// VA VistA selectors (verified YYYY-MM-DD)
...
VA VistA updates its UI periodically. When selectors break:
