/**
 * anchor-cloud.js — AnchorBrowser cloud example for VA VistA
 *
...
const { login_va_access, schedule_appointment } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting VA VistA automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_va_access(page, {});
      log('Action 1 result:', data1);

      const data2 = await schedule_appointment(page, {});
