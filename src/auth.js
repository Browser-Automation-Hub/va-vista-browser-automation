/**
 * auth.js — Authentication handler for VA VistA
 * Supports: SSO (VA Access / ID.me), MFA (ID.me / Login.gov / PIV Card)
 *
...
const BASE_URL = process.env.VA_VISTA_URL || 'https://eauth.va.gov/accessva/';
const USERNAME = process.env.VA_VISTA_USERNAME;
const PASSWORD = process.env.VA_VISTA_PASSWORD;
...
async function performLogin(page) {
  if (!USERNAME || !PASSWORD) {
    throw new Error(`Set VA_VISTA_USERNAME and VA_VISTA_PASSWORD in .env`);
  }

  console.log('Logging in to VA VistA...');
  await page.goto(BASE_URL, { waitUntil: 'networkidle2' });
